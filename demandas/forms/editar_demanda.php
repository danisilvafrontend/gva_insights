...
// Todos os usuários logados podem editar qualquer demanda
// Admin é o único que pode trocar o responsável
...
$categorias = [
    'Gestão & Planejamento' => ['Cronograma','Locais','Metas (clientes, marcas, números de pessoas)','Parcerias','Controle e Follow Up'],
    'Comunicação'          => ['Videos Promo','Webinars','Releases Brasil','Releases EUA','Newsletter','Posts SoMe','Plataforma'],
    'Documentação'         => ['Contratos','Invoice','Acordos','Clientes','Parceiros','Fornecedores'],
    'Organização e Execução' => ['Roadshow Presencial','Roadshow Virtual','Eventos Especiais','Agenda B2B','Travel Arrangements','Promoção e RSVP'],
    'Relatórios'            => ['Template de Relatórios','Atualização de Dados','Entrega de Relatórios'],
];

$meses = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];

$catConteudo = [
    'Comunicação › Videos Promo','Comunicação › Webinars','Comunicação › Releases Brasil',
    'Comunicação › Releases EUA','Comunicação › Newsletter','Comunicação › Posts SoMe','Comunicação › Plataforma',
];
...
<head>
    ...
    <title>Editar Demanda #<?= $id ?></title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <link rel="stylesheet" href="../assets/demandas.css">
    <style>
        .chip-select-group { display:flex; flex-wrap:wrap; gap:8px; padding:6px 0; }
        ...
        .chip-select-group label {
            ...
            font-size:.83rem; cursor:pointer; background:#fff; color:#555;
            transition:all .15s ease; user-select:none; line-height:1.4;
        }
        .chip-select-group label::before {
            content:''; display:inline-block; width:13px; height:13px;
            border:1.5px solid #aaa; border-radius:3px; background:#fff;
            flex-shrink:0; transition:all .15s ease;
        }
        .chip-select-group input[type="checkbox"]:checked + label { background:#e8f0fe; border-color:#0d6efd; color:#0d6efd; font-weight:500; }
        ...
        .chip-label-section { font-size:.78rem; font-weight:600; text-transform:uppercase; letter-spacing:.04em; color:#6c757d; margin-bottom:2px; }
          ...
          <div class="row g-3">
            <div class="col-md-12">
              ...
              <div class="chip-select-group">
                ...
                <input type="checkbox" name="empresas_envolvidas[]" id="emp_<?= $emp['id'] ?>" value="<?= $emp['id'] ?>" <?= in_array((int)$emp['id'], $empSelecionados) ? 'checked' : '' ?>>
                <label for="emp_<?= $emp['id'] ?>"><?= htmlspecialchars($emp['empresa']) ?></label>
                <?php endforeach; ?>
                <?php if (empty($empresas)): ?><span class="text-muted small">Nenhuma empresa cadastrada.</span><?php endif; ?>
              </div>
            </div>
            <div class="col-md-12">
              ...
              <div class="chip-select-group">
                ...
                <input type="checkbox" name="clientes_envolvidos[]" id="cli_<?= $cli['id'] ?>" value="<?= $cli['id'] ?>" <?= in_array((int)$cli['id'], $cliSelecionados) ? 'checked' : '' ?>>
                <label for="cli_<?= $cli['id'] ?>"><?= htmlspecialchars($cli['company']) ?></label>
                <?php endforeach; ?>
                <?php if (empty($clientes)): ?><span class="text-muted small">Nenhum cliente cadastrado.</span><?php endif; ?>
              </div>
            </div>
          </div>
...
document.getElementById('categoria').addEventListener('change', function () {
  const isC = CAT_CONTEUDO.includes(this.value);
  document.getElementById('fields-conteudo').classList.toggle('d-none', !isC);
  document.getElementById('fields-gestao').classList.toggle('d-none',    isC);
});
</script>
</body>
</html>
