...
<meta charset="UTF-8">
    <div class="container">
        <div class="row">
            <div class="col-6">
                <a href="https://insights.gvacompany.com/views_bd/views_ondemand.php">Voltar</a>
            </div>
        </div>
    </div>

    <div class="container">
        <h2 class="my-4">Editar Registro On-Demand</h2>
        <?php
        ...
        if (isset($_GET['id'])) {
            ...
            $sql = "SELECT * FROM bm_ondemand WHERE id = $id";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                $row = $result->fetch_assoc();
                ?>
                <form action="../forms/update_ondemand.php" method="POST">
                    ...
                    <div class="form-group form-floating mt-2">
                        <select class="form-control" id="evento_id" name="evento_id" required>
                            ...
                            $sql_eventos = "SELECT id, evento FROM bm_lives";
                            $result_eventos = $conn->query($sql_eventos);

                            if ($result_eventos->num_rows > 0) {
                                while ($evento = $result_eventos->fetch_assoc()) {
                                    echo "<option value='" . $evento['id'] . "' " . ($evento['id'] == $row['evento_id'] ? 'selected' : '') . ">" . $evento['evento'] . "</option>";
                                }
                            }
                            ?>
                        </select>
                    ...
                    <div class="form-group form-floating mt-2">
                        <input type="date" class="form-control" id="data_relatorio" name="data_relatorio" value="<?php echo $row['data_relatorio']; ?>" required>
