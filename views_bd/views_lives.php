...
<div class="container">
    ...
    $sql = "SELECT bm_lives.*, 
            ...
            FROM bm_lives 
            JOIN clientes ON bm_lives.empresa_id = clientes.id 
            JOIN bm_canais ON bm_lives.canal_id = bm_canais.id
            WHERE 1=1";
    ...
    if ($filtro_cliente > 0) {
        $sql .= " AND bm_lives.empresa_id = ?";
        $params[] = $filtro_cliente;
        $types .= 'i';
    }

    if ($filtro_canal > 0) {
        $sql .= " AND bm_lives.canal_id = ?";
        $params[] = $filtro_canal;
        $types .= 'i';
    }

    if (!empty($filtro_mes)) {
        // filtra por ano-mês da data_evento
        $sql .= " AND DATE_FORMAT(bm_lives.data_evento, '%Y-%m') = ?";
        $params[] = $filtro_mes;
        $types .= 's';
    }

    $sql .= " ORDER BY bm_lives.data_evento DESC LIMIT 50";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>$data_evento</td>
                    <td>{$row['inscritos']}</td>
                    <td>{$row['peak_views']}</td>
                    <td>{$row['visualizacoes']}</td>
                    <td><a href='../forms/edit_lives.php?id={$row['id']}' class='btn btn-primary btn-sm'>Editar</a></td>
                  </tr>";
        }
        echo "</tbody></table>";
    } else {
